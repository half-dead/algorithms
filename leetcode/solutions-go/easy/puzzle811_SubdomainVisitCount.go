...
func subdomainVisits(cpdomains []string) []string {
  dict := make(map[string]int, len(cpdomains)*3)
  for _, cpd := range cpdomains {
    spaceIndex := strings.IndexRune(cpd, ' ')
    count, _ := strconv.Atoi(cpd[:spaceIndex])

    cpd = cpd[spaceIndex+1:]
    for ; len(cpd) > 0; {
      dict[cpd] += count
      dotIndex := strings.IndexRune(cpd, '.')
      if dotIndex != -1 {
        cpd = cpd[dotIndex+1:]
      } else {
        cpd = ""
...
func main() {
  fmt.Println(subdomainVisits([]string{"9000 shit.google.com"}))
}
