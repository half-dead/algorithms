 ...
 * @author half-dead
 */
public class Puzzle1462 {
    public static void main(String[] args) {
        Solution s = new Puzzle1462().new Solution();
        System.out.println(s.checkIfPrerequisite(33, new int[][]{
    ...
    class Solution {
        public List<Boolean> checkIfPrerequisite(int n, int[][] edges, int[][] queries) {
            ...
            for (int[] q : queries) list.add(g[q[0]][q[1]]);
            return list;
        }
    }

    // backwarding backtrack, TLE
    class BackTrackingSolution {
        public List<Boolean> checkIfPrerequisite(int n, int[][] edges, int[][] queries) {
            ...
            Map<Integer, Set<Integer>> g = new HashMap<>(), f = new HashMap<>();
            for (int i = 0; i < n; i++) {
                ends.add(i);
                g.put(i, new HashSet<>());
                f.put(i, new HashSet<>());
            ...
            for (int end : ends) bt(end, new LinkedList<>(), g, f);
            for (int[] q : queries) result.add(f.get(q[0]).contains(q[1]));
            return result;
        }

        void bt(int course, LinkedList<Integer> curr, Map<Integer, Set<Integer>> g, Map<Integer, Set<Integer>> f) {
            curr.addLast(course);
            for (int prev : g.get(course)) {
                f.get(prev).addAll(curr);
                bt(prev, curr, g, f);
            }
            curr.removeLast();
        }
    ...
    class BFSSolution {
        public List<Boolean> checkIfPrerequisite(int n, int[][] prerequisites, int[][] queries) {
            ...
            for (int[] p : prerequisites) g.get(p[0]).add(p[1]);

            for (int start = 0; start < n; start++) {
                Set<Integer> reachable = g.get(start);
                boolean[] visited = new boolean[n];
                Queue<Integer> q = new LinkedList<>(reachable);
            ...
            for (int[] q : queries) result.add(g.get(q[0]).contains(q[1]));
            return result;
        }
    }
}
