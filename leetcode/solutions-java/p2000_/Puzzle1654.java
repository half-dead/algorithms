 ...
 * @author half-dead
 */
public class Puzzle1654 {

    public static void main(String[] args) {
        Solution s = new Puzzle1654().new Solution();
        System.out.println(s.minimumJumps(new int[]{
                14, 4, 18, 1, 15
        }, 3, 15, 9));
        System.out.println(s.minimumJumps(new int[]{
                8, 3, 16, 6, 12, 20
    ...
    class Solution {
        public int minimumJumps(int[] forbidden, int a, int b, int x) {
            ...
            int limit = 2001 + 2 * Math.max(a, b);
            boolean[] fbd = new boolean[limit];
            for (int n : forbidden) {
                fbd[n] = true;
            }
            ...
            while (dq.size() > 0) {
                ...
                while (len-- > 0) {
                    int v = dq.pollFirst();
                    int pos = Math.abs(v);
                    int fp = pos + a, bp = pos - b, cnt = m[pos] + 1;

                    if (fp < limit && !fbd[fp]) {
                        if (mem.add(fp)) {
                            dq.addLast(fp);
                        }
                        m[fp] = m[fp] < 0 ? cnt : Math.min(m[fp], cnt);
                    }

                    if (v > 0 && bp > 0 && bp < limit && !fbd[bp]) {
                        if (mem.add(-bp)) {
                            dq.addLast(-bp);
                        }
                        m[bp] = m[bp] < 0 ? cnt : Math.min(m[bp], cnt);
                    }
                }
            }
