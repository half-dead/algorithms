...
public class Puzzle1976 {
    ...
    class Solution {
        public int countPaths(int n, int[][] roads) {
            int mod = (int) 1e9 + 7;
            // shortest time to reach i from 0
            long[] times = new long[n];
            // number of ways to reach i from 0 in the shortest time
            long[] ways = new long[n];

            // build graph
            // further more, we could store time costs of every edge in a 2-D array to save some memory
            Map<Integer, Map<Integer, Integer>> g = new HashMap<>();
            for (int[] road : roads) {
                g.computeIfAbsent(road[0], x -> new HashMap<>()).put(road[1], road[2]);
                g.computeIfAbsent(road[1], x -> new HashMap<>()).put(road[0], road[2]);
            }

            boolean[] visited = new boolean[n];
            // element structure: [to_id, from_id, time_cost]
            PriorityQueue<long[]> q = new PriorityQueue<>((x, y) -> (int) (x[2] - y[2]));
            q.offer(new long[]{0, 0, 0});
            while (q.size() > 0) {
                long[] prev = q.poll();
                int id = (int) prev[0], pid = (int) prev[1];
                long time = prev[2];
                // number of ways to reach pid from zero in shortest time
                long waysToReachPid = pid == 0 ? 1 : ways[pid];

                // first time reach `id`
                if (!visited[id]) {
                    visited[id] = true;
                    times[id] = time;
                    ways[id] = (ways[id] + waysToReachPid) % mod;
                } else {
                    // reached at `id` more than once in shortest time, need to increment ways[id]
                    if (time == times[id]) ways[id] = (ways[id] + waysToReachPid) % mod;
                    // no need to go further from `id`, because it's already in the Q
                    continue;
                }

                Map<Integer, Integer> next = g.get(id);
                if (next == null || next.size() == 0) continue;

                for (int nid : next.keySet()) {
                    if (visited[nid]) continue;
                    q.offer(new long[]{nid, id, time + next.get(nid)});
                }
            }
            return (int) (ways[n - 1]);
        }
        ...
        public int countPaths(int n, int[][] roads) {
            ...
            while (!pq.isEmpty()) {
                ...
                for (int[] nei : graph.get(u)) {
                    ...
                    if (curr[1] + nei[1] < dist[v]) {
                        dist[v] = curr[1] + nei[1];
                        ways[v] = ways[u];
                        pq.offer(new long[] {v, dist[v]});
                    } else if (curr[1] + nei[1] == dist[v]) {
