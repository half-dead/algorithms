 ...
 * @author half-dead
 */
public class Puzzle2071 {

    public static void main(String[] args) {
        Solution s = new Puzzle2071().new Solution();
        System.out.println(s.maxTaskAssign(new int[]{
        ...
        System.out.println(s.maxTaskAssign(new int[]{5, 4}, new int[]{0, 0, 0}, 1, 5));
    }

    // binary search, greedy, monotonic stack or tree map
    class Solution {
        public int maxTaskAssign(int[] tasks, int[] workers, int pills, int strength) {
            ...
            while (lo < hi) {
                int mid = (lo + hi + 1) / 2;
                boolean b = check(mid, tasks, workers, pills, strength);
                if (b) {
                    lo = mid;
                } else {
                    hi = mid - 1;
        ...
        boolean check(int k, int[] t, int[] w, int p, int s) {
            Deque<Integer> dq = new ArrayDeque<>();
            for (int i = k - 1, j = w.length - 1; i >= 0; i--) {
                while (j >= w.length - k && w[j] + s >= t[i]) {
                    dq.addFirst(w[j--]);
                }
